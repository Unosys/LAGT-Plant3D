...
namespace Autodesk.GUIHarness.Plant3D.Dialogs
{
    public partial class UserConflictDLG : Autodesk.GUIHarness.AutoCAD.AcDialogBox
    {
		...
		{
			Tag = "[DialogBox]" + AcMainWin.cBabel(23165,17,"User Conflict");
		}
		...
		public override void Invoke()
		{
            ...
            try
            {
                PROJECTMANAGERDLG.CurrentProjectPL.Select("TestVault");
               
            }
            catch
            {
                PROJECTMANAGERDLG.OpenPB.Click();
                PROJECTMANAGERDLG.OpenPB.TypeKeys("<Home>T<Enter>", 250);
		...
		{
            Current<UserConflictDLG>().ClosePB.Click();
            Drawing.Current.TypeE("_PLANTVAULTLOGOUT");
            VaultLogOutDLG VaultLogOutDLG = new VaultLogOutDLG();
            VaultLogOutDLG.LogInPB.Click();
            VaultLogInDLG VaultLogInDLG = new VaultLogInDLG();
            VaultLogInDLG.UserNameTextBox.ClearText();
